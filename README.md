...
- ### STUPID code, seriously? Don't do like this                                                                                                                                                                
    - S Singleton
    - T Tight Coupling
    - U Untestability
    ...
    - D Duplication
- ### LIQUID programming.  Don't do like this 
...
## Objects
  ...
  - [3.8](#objects--rest-spread) Prefer the object spread operator over [`Object.assign`](https://developer.mozilla.org/en/docs/Web/JavaScript/Reference/Global_Objects/Object/assign) to shallow-copy objects. Use the object rest operator to get a new object with certain properties omitted.
    ...
    // good in some cases. 
    // When you need to cut prototype properties or object references
    // When you need the fastest way to deeply clone
    const copy = {...JSON.parse(JSON.stringify(original)), 3}    
...
## Functions
  ...
  - [7.1](#functions--declarations) Use named function expressions instead of function declarations. eslint: [`func-style`](https://eslint.org/docs/rules/func-style)
    ...
    // good
    // the best way to avoid creating a long name is to organize the code
    const alias = instanceName.methodName
    ```
  ...
  - [7.15](#functions--signature-invocation-indentation) Functions with multiline signatures, or invocations, should be indented just like every other multiline list in this guide: with each item on a line by itself, with a trailing comma on the last item. eslint: [`function-paren-newline`](https://eslint.org/docs/rules/function-paren-newline)
    ...
    // it’s better not to use multi-line arguments and try to make fewer lines
    function(bar, baz, quuz, ...,
                lastOne) {
        // ...
    }
    ```
...
## Arrow Functions
  ...
  - [8.3](#arrows--paren-wrap) In case the expression spans over multiple lines, wrap it in parentheses for better readability.
    ...
    // good
    ['get', 'post', 'put'].map((httpMethod) => Object
        .prototype.hasOwnProperty.call(httpMagicObjectWithAVeryLongName, httpMethod));
    ```
...
## Classes & Constructors
  ...
  The request not to override the native method is confusing
...
## Iterators and Generators
  ...
  - [11.1](#iterators--nope) Use integrators only to wait for promises. Without this, we cannot wait for a few promises. We must to disable this rules: eslint: [`no-iterator`](https://eslint.org/docs/rules/no-iterator.html) [`no-restricted-syntax`](https://eslint.org/docs/rules/no-restricted-syntax)
...
## Properties
  ...
  - [12.1](#properties--dot) Use dot notation when accessing properties. We must to disable this rule: eslint: [`dot-notation`](https://eslint.org/docs/rules/dot-notation.html)
...
## Variables
  ...
  - [13.6](#variables--unary-increment-decrement) Avoid using unary increments and decrements (`++`, `--`). eslint [`no-plusplus`](https://eslint.org/docs/rules/no-plusplus)
    ####FYI:  valid only when we do not use a semicolon (;)
...
## Comparison Operators & Equality
  ...
  Make for short and make for clear
    ...
    // realy bad
    if (zero === 0 && notNumber === NaN && booleanFalse === false) {
      // ...
    }
    
    // good
    if(zero && notNumber && booleanFalse) {
      // ...
    }
    ```
  ...
  - [15.5](#comparison--switch-blocks) Use braces to create blocks in `case` and `default` clauses that contain lexical declarations (e.g. `let`, `const`, `function`, and `class`). eslint: [`no-case-declarations`](https://eslint.org/docs/rules/no-case-declarations.html)
    ###FYI: Only for declarations, not for everything
...
## Blocks
  ...
  - [16.1](#blocks--braces) ~~Use braces with all multiline blocks.~~ We must disable this rule eslint: [`nonblock-statement-body-position`](https://eslint.org/docs/rules/nonblock-statement-body-position)
    > We must make the code as short as possible and always so that it is understandable to the reader.
    ...
    // bad
    function foo() { some code; return false; }
  ...
  - [16.3](#blocks--no-else-return) If an `if` block always executes a `return` statement, the subsequent `else` block is unnecessary. A `return` in an `else if` block following an `if` block that contains a `return` can be separated into multiple `if` blocks. eslint: [`no-else-return`](https://eslint.org/docs/rules/no-else-return)
    > Avoid to use return in if block. In byte code each if without else have empty else. Each if and else have their own return in byte code. A function or method should have only one return, otherwise the code will be difficult to maintain and debug.
...
## Control Statements
  ...
  - [17.1](#control-statements) In case your control statement (`if`, `while` etc.) gets too long or exceeds the maximum line length, each (grouped) condition could be put into a new line. The logical operator should begin the line.
    ...
    Avoid using long conditions. Hide the long condition under the hood of class methods or functions.
    ...
    // best
    if (this.isCondition()) thing1();
    
    // best
    if(className.isCondition()) thing1();
    
    // awesome
    this.isCondition() && thing1();
    
    // also cool
    this.isCondition() ? thing1() : thing2();
...
## Whitespace
  ...
  - [19.1](#whitespace--spaces) Use soft tabs (space character) set to 4 spaces. eslint: [`indent`](https://eslint.org/docs/rules/indent.html)
    ...
    // bad, but we ready to discussion
    function baz() {
    ∙∙let name;
    }
    ```
...
## Commas
  ...
  - [20.2](#commas--dangling) Additional trailing comma: **Yup.** eslint: [`comma-dangle`](https://eslint.org/docs/rules/comma-dangle.html)
    ...
    ####FYI: Please don't use trailing coma without babel
...
## Semicolons
  ...
  - [21.1](#semicolons--required) ~~**Yup.**~~ We must to daisable this rule eslint: [`semi`](https://eslint.org/docs/rules/semi.html)
    ...
    #### FYI: We don't use malfunctional peace of code, which was described above, in projects, and don't use semicolon.
    ```javascript
...
## Naming Conventions
  ...
  ####FYI : Some os don't support camel case names, we use kebab-case for file-names.
...
## Accessors
  ...
  ### FYI: We are in 2020 and should't support es5. This is not a strict rule, if you can use setters and getters and make clear code, feel free to use it.
...
## Testing
  ...
  - [30.2](#testing--for-real) **No, but seriously**:
    ...
    - We primarily use [`mocha`](https://www.npmjs.com/package/mocha) and [`chai`](https://www.npmjs.com/package/chai) at Airbnb. 
